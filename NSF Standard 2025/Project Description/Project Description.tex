...
Another approach one may take towards Goal~\ref{goal:stackyGreens} is by studying the algebraic properties of the canonical rings $R(\cX;K_{\cK})$. As a first step towards this, together with Maya Banks and Christopher Manon, we have shown that just like normal non-stacky curves, the canonical ring of stacky curves is Gorenstein.  
...
I plan to develop a \textit{Macaulay2} package \textit{toricStacks} implementing the necessary data structure and methods for working with toric stacks. This includes implementing: stacky fans, morphisms of stacky fans, line bundles on toric stacks, the cox ring of a toric stack, and cohomology computations involving sheaves on toric stacks. Work on such a package began in Summer 2025 with Maya Banks and John Cobb. The PI has extensive experience developing \textit{Macaulay2} packages and will involve graduate students in this project. The package will be open-source.


\begin{problem}
	Compute the graded Betti tables of $\cP(1,1,1,a)$ embedded by $\cO(d)$ for all $1<a \leq 15$ and all $2\leq d \leq 6$ by adapting the high-performance computing approaches developed in \juliette{BEGY20,BCEGLY22}. 
\end{problem}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Castelnuovo–Mumford Regularity: Resolutions, Bounds, and Asymptotic Behavoir}\label{sec:multigraded-regularity}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A central invariant measuring the complexity of a $\Pic(Y)$-graded $S$-module $M$ or a is coherent sheaf $\cF$ on $Y$ is the multigraded  Castelnuovo-Mumford regularity (or just multigraded regularity) of $M$ or $\cF$. (See Section \juliette{NEEED} for the definition. Crucially, $\reg(M)$ is not a single degree, but a region in $\Pic(X)$. Unlike in the classical setting, even for relatively simple examples the multigraded regularity does not necessarily have a unique minimal element (see Figure~\ref{fig:example-of-reg}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \subsection{Asymptotics and Bounds on Multigraded Regularity}
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We call the set on the right hand side of Conjecture~\ref{conj:reg-ideal-sheaves} the Seshadri region of $\cI$ as it is a natural multigraded analog \juliette{NEDEDEDE}.  This conjecture suggests that the asymptotic regularity has many surprising and nice properties. For example, depot $\reg(\cI^p)$ rarely being convex the Seshadri region is convex. 
...
\end{figure}
 ...
 \juliette{ADDD SOMETHING HERE}
 
  \begin{problem}
 	Is the Seshadri regions defined above be determined by the Newton-Okunkov body of some flag on $Y$. 
 \end{problem}
 
In addition to understanding the asymptotic limit of $\reg(I^p)$ and $\reg(\cI^p)$ is would be interesting to attempt to refine our understanding of the growth of the regularity in the settings. In particular, based on computations -- and as generally implied by Conjecture~\ref{conj:reg-ideal-sheaves} -- the number of minimal generator -- i.e. ``corners''  of $\reg(I^p)$ and $\reg(\cI^p)$, with respect to the $\Nef(Y)$ ordering, seems to grown with $p$. This is a phenomena new to the multigraded setting since on $\PP^n$ the regularity is always generated by a single element. It would be interesting to try to better understand this growth.

 \begin{problem}\label{prob:corners}
 	Let $Y$ be a smooth projective toric variety with $S$ it's $\Pic(Y)$-graded Cox ring. Le If $I\subset S$ is a $\ZZ^r$-graded ideal and $\cI \subset \cO_{Y}$ and ideal sheaf on $Y$. 
	\begin{enumerate}
	\item Bound the number of minimal generators of $\reg(I^p)$ as a function of $p$.
	\item Bound the number of minimal generators of $\reg(\cI^p)$ as a function of $p$.
	\end{enumerate}
\end{problem}
...
Despite being a relatively simple class of subvarieties it has been shown that their multigraded regularity behaves in nuanced ways \juliette{needED} and more generally many simple results fail to generalize to multigraded regularity in ``easy'' ways. \juliette{needED}


Lastly as a somewhat losely connected project, I would like to explore the following specific example. The Deligne-Mumford moduli space of $n$-pointed stable curves of genus zero, denoted by $\overline{M}_{0,n}$, has long been of interest because of its rich geometry and combinatorics \cite{knudsen83,GHvdP88,keel92,kerberMarkwig09,manin13,CGM21,GGL23}. Building upon work of Kapranov \cite{kapranov93}, Tevelev and Tevelev showed there is an embedding  $\Omega_{n}:\overline{M}_{0,n} \hookrightarrow \PP^{1}\times \PP^{2}\times \cdots \times \PP^{n-3}$, through which the log canonical embedding $\partial \overline{M}_{0,n} + K_{\overline{M}_{0,n}}$ naturally factors \cite{keelTevelev09}. Monin and Rana \cite{moninRana17} produced a set of equations, arising as the $2\times 2$ minors of certain matrices of linear forms, they conjectured defined the scheme theoretic image of $\Omega_{n}$, this was recently prove true by Gillepsie, Griffin, and Levinson \cite{GGL22}. 
...
Using \textit{Macaulay2} the regularity and free resolution of $\Omega_{n}(\overline{M}_{0,n})$ is computable for $n=4,5,6,7$. I hope to take two-step approach two answering Problem~\ref{prob:m0n} two-fold: First, leverage the fact that $\Omega_{n}(\overline{M}_{0,n})$ is \juliette{ADDD}. Second, use mapping cone constructions to convert the resolution computed in part one to  \juliette{ADD}
Lastly, it is of interest to note that this \juliette{ADD}
...
Note the PI  provided an answer to this question when $Y$ is a product of projective spaces \cite[Theorem 6.2]{bruceHellerSayrafi21}. 
...
This question is open in all cases except $Y=\PP^n$. The difficulty to this is finding a box, in which one is guaranteed all the minimal generators of $\reg(M)$ live. Right now most bounds look like $L$-shaped regions, i.e. translates of $\Nef(X)$, which does not constraint the search space of minimal generators of $\reg(M)$. In this way finding an answer to Problem~\ref{prob:mg-reg-alg} is closely related to the question of questions around bounding multigraded regularity proposed in this project. 
