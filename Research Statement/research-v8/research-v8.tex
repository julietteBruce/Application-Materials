...
Let $X\subset \P^{r_d}$ be a smooth projective variety, $\dim X \geq2$, and fix an index $1\leq q \leq n$. Let $(L_{d})_{d\in\N}$ be a sequence of very ample line bundles such that $L_{d+1}-L_{d}$ is constant and semi-ample compute $\lim_{d\to\infty} \rho_{q}\left(X;L_{d}\right)$?
\end{question}
