...
Generically if one picks two polynomials $f,g\in \C[x,y,z]$ their common zero locus in $\P^3$ will be an algebraic curve. Curves arriving in such a way are called complete intersections. Classically one approach to studying the geometry of curves in $\P^3$ is by asking when the union of two (or more) curves is a complete intersection. Such curves are said to be linked, and the general idea of liaison theory is to try and identify properties that are preserved under linkage. 
